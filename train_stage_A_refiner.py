...
from cascade import DecoderStageA, EncoderStageA, RefinerStageA
...
def init_model(batch_size = 256, training_res = 256, latent_dim=4, compression_ratio=4, seed = 42, learning_rate = 10e-3):
    with jax.default_device(jax.devices("cpu")[0]):
        enc_rng, dec_rng, lpips_rng, refiner_rng = jax.random.split(jax.random.PRNGKey(seed), 4)
        ...
        refiner = RefinerStageA(
            output_features = 3,
            upscale_factor = 1,
            dim = 48,
            kernel_size = 3,
            numb_resnet_blocks = 10,
            use_bias = False ,
            expansion_factor = 4,
            eps = 1e-6,
            group_count = -1,
        ...
        refiner_params = refiner.init(refiner_rng, image)
        # LPIPS VGG16
        ...
        refiner_param_count = sum(list(flatten_dict(jax.tree_map(lambda x: x.size, refiner_params)).values()))
        print("encoder param count:", enc_param_count)
        print("decoder param count:", dec_param_count)
        print("refiner param count:", refiner_param_count)
        ...
        refiner_state = train_state.TrainState.create(
            apply_fn=refiner.apply,
            params=refiner_params,
            tx=adam_wrapper(
                jax.tree_util.tree_map_with_path(lambda path, var: path[-1].key != "scale" and path[-1].key != "bias", refiner_params)
            ),
        )
        ...
        return [refiner_state, enc_state, dec_state, lpips_state]
...
def train_vae_only(models, batch, loss_scale, train_rng):
    ...
    # unpack model
    refiner_state, enc_state, dec_state, lpips_state = models

    def _vae_loss(refiner_params, enc_params, dec_params, lpips_params, batch, loss_scale, vae_rng):
        # cast input
        refiner_params, enc_params, dec_params, lpips_params, batch = mixed_precision_policy.cast_to_compute((refiner_params, enc_params, dec_params, lpips_params, batch))
        latents = enc_state.call(enc_params, batch)
        # deliberate lossy compression
        # latents = latents.astype(jnp.float8_e5m2).astype(jnp.bfloat16)
        image_raw = dec_state.call(dec_params, latents)
        pred_batch = refiner_state.apply_fn(refiner_params, image_raw)
    ...
    new_refiner_state = refiner_state.apply_gradients(grads=jmp.cast_to_full(vae_grad[0]))

    # pack models 
    new_models_state = new_refiner_state, enc_state, dec_state, lpips_state
...
def inference(models, batch):
    refiner_state, enc_state, dec_state, _ = models

    refiner_params, enc_params, dec_params, batch = mixed_precision_policy.cast_to_compute((refiner_state.params, enc_state.params, dec_state.params, batch))
    latents = enc_state.call(enc_params, batch)
    image_raw = dec_state.call(dec_params, latents)
    return refiner_state.apply_fn(refiner_params, image_raw)

def main():
    BATCH_SIZE = 128
    SEED = 0
    EPOCHS = 100
    SAVE_MODEL_PATH = "refiner"
    ...
    if LOAD_CHECKPOINTS != 0:
        ...
        refiner_params = unflatten_dict(load_file(f"{SAVE_MODEL_PATH}/{STEPS}/refiner_params.safetensors"))
        models[0].params.update(jax.tree_map(lambda leaf: jax.device_put(leaf, device=NamedSharding(mesh, PartitionSpec())), refiner_params))
        del refiner_params

        try:
            refiner_mu = unflatten_dict(load_file(f"{SAVE_MODEL_PATH}/{STEPS}/refiner_mu.safetensors"))
            refiner_nu = unflatten_dict(load_file(f"{SAVE_MODEL_PATH}/{STEPS}/refiner_nu.safetensors"))
            models[0].opt_state[1][0].mu.update(jax.tree_map(lambda leaf: jax.device_put(leaf, device=NamedSharding(mesh, PartitionSpec())), refiner_mu))
            models[0].opt_state[1][0].nu.update(jax.tree_map(lambda leaf: jax.device_put(leaf, device=NamedSharding(mesh, PartitionSpec())), refiner_nu))
            del refiner_mu, refiner_nu
    ...
    models[1].params.update(jax.tree_map(lambda leaf: jax.device_put(jmp.cast_to_half(leaf), device=NamedSharding(mesh, PartitionSpec())), enc_params))
    del enc_params

    dec_params = unflatten_dict(load_file(f"{SAVE_MODEL_PATH}/{STEPS}/dec_params.safetensors"))
    models[2].params.update(jax.tree_map(lambda leaf: jax.device_put(jmp.cast_to_half(leaf), device=NamedSharding(mesh, PartitionSpec())), dec_params))
    del dec_params
        ...
        for i, batch in enumerate(t_dl):
            ...
            if STEPS % SAVE_EVERY == 0:
                ...
                try:
                    ...
                    save_file(flatten_dict(models[0].params), f"{SAVE_MODEL_PATH}/{STEPS}/refiner_params.safetensors")
                    save_file(flatten_dict(models[0].opt_state[1][0].mu), f"{SAVE_MODEL_PATH}/{STEPS}/refiner_mu.safetensors")
                    save_file(flatten_dict(models[0].opt_state[1][0].nu), f"{SAVE_MODEL_PATH}/{STEPS}/refiner_nu.safetensors")
                    save_file(flatten_dict(models[1].params), f"{SAVE_MODEL_PATH}/{STEPS}/enc_params.safetensors")
                    save_file(flatten_dict(models[2].params), f"{SAVE_MODEL_PATH}/{STEPS}/dec_params.safetensors")
